...
interface CardsInterface {
    data: OptionsVariationInterface[] | null;
    deleteVariation;
    setDataList;
    deleteColor;
}

const ColorsCards = ({ data, deleteVariation, deleteColor, setDataList }: CardsInterface) => {
        ...
        <>
            {openModal && (
                <DeleteModal
                    ...
                    string={string}
                    text={string?.do_you_want_to_delete_color}
                    action={() =>
                        deleteVariation({ id: selectedVariationId, storeCode })
                            .then(res => {
                                if (res.status === 200)
                                    deleteColor({ id: selectedColorId, storeCode })
                                        .then(res => {
                                            if (res.status === 200) toast.success(string?.deleted);
                                            setDataList(data.filter(el => el.variationId !== selectedVariationId));
                                        })
                                        .catch(err => {
                                            console.log(err);
                        ...
                        <Grid xs={sx ? 12 : 2} sx={{ ml: 'auto', p: 1, borderTop: sx ? '1px solid #ccc' : '' }}>
                            ...
                            >
                                <IconButton
                                    ...
                                    size="small"
                                    onClick={() => {
                                        navigate(`/store-inventory/${storeCode}/options/colors/${item?.colorId}/edit`);
                                    }}
                                >
